...
APIGet(ServiceURL.Transaction.unpaid(getCookie("id"))).then(res => {
    ...
    if (data.maxDueDate > 0) maxdocument.querySelector(".due-date").innerHTML = `Tanggal Jatuh Tempo <span class="fw-bold due-date">${UNIXtimeConverter(data.maxDueDate, "DD MMMM YYYY")}</span>`
})
